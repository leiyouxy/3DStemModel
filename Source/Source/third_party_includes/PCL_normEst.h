...
//template Class
template< class PointXYZRGB, class Normal>
class PCL_Normal_Estimator{
...
public:
	 ...
	 * @param normals
	 */
	PCL_Normal_Estimator(typename pcl::PointCloud<PointXYZRGB>::Ptr points, 
	...
	typename pcl::PointCloud<PointXYZRGB>::Ptr& point_cloud(){return pts;}
	typename pcl::PointCloud<PointXYZRGB>::Ptr point_cloud()const{return pts;}
	...
	void estimate(int method= POINTS, int neighborhood_type= KNN, float neighborhood_size= 100){
...
private:
	...
	typename pcl::PointCloud<PointXYZRGB>::Ptr pts;/*!< Point cloud*/
	...
	inline void assign_points_to_cubes(std::vector<int> &pointIdxRadiusSearch, std::vector<int> cubes[],
			float radius, PointXYZRGB &refPoint, PointXYZRGB points[],
		...
		for(unsigned int i=0; i<pointIdxRadiusSearch.size(); i++){
			...
			PointXYZRGB refPoint2 = refPoint;
	 ...
	 */
	inline void find_a_triplet(pcl::KdTreeFLANN<PointXYZRGB> &tree, float radius1, float radius2, Eigen::Vector3i &triplet,
		PointXYZRGB &pt){
	...
	inline void generate_list_of_triplets(std::vector<Eigen::Vector3i> &triplets,
			...
			float radius2,pcl::KdTreeFLANN<PointXYZRGB> &tree,
		PointXYZRGB & pt,
	...
	void points_knn(int neighbor_number){
		...
		for(int per=0; per<(int)pts->size(); per++){
			...
			PointXYZRGB *points = new PointXYZRGB[points_size];
	...
	void unif_knn(int neighbor_number){
		...
		for(int per=0; per<(int)pts->size(); per++){
			...
			typename pcl::PointCloud<PointXYZRGB>::Ptr cloud_neighbors (new pcl::PointCloud<PointXYZRGB>);
			cloud_neighbors->resize(pointIdxRadiusSearch.size());
			for(unsigned int i=0; i<pointIdxRadiusSearch.size(); i++){
				cloud_neighbors->points[i] = pts->points[pointIdxRadiusSearch[i]];
			}
			pcl::KdTreeFLANN<PointXYZRGB> tree_neighbors;
			tree_neighbors.setInputCloud(cloud_neighbors);

			PointXYZRGB *points = new PointXYZRGB[neighbor_number];
